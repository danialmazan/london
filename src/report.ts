...
const partyColours: Record<string, string> = { labour: "#E4003B", conservative: "#0087DC", liberal_democrat: "#FAA61A", green: "#6AB023", reform: "#12B6CF" };
const groups = {
...
function electionRows(item: ReportElection): string {
  const shares = Object.entries(item.shares).filter(([, value]) => value !== null).sort((a, b) => (b[1] ?? 0) - (a[1] ?? 0));
