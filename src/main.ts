...
function createMap(): void {
  ...
  map.addControl(
    new maplibregl.AttributionControl({
      compact: true,
      customAttribution: "London Atlas · Daniel Almazán",
    }),
    "bottom-left",
  );
...
function addCanonicalSectionHitLayer(): void {
  const source = manifest.sources.find((candidate) => candidate.id === "lsoa21");
      ...
      map.addLayer(
        {
          ...
          type: "circle",
          paint: {
            "circle-radius": ["interpolate", ["linear"], ["zoom"], 11, 1.3, 16, 4],
            "circle-color": colorExpression(definition),
            "circle-opacity": definition.opacity ?? 0.88,
            "circle-stroke-width": ["interpolate", ["linear"], ["zoom"], 11, 0, 15, 0.6],
...
function routeFilter(definition: LayerDefinition): ExpressionSpecification | undefined {
  const mode = definition.control?.transportMode;
  if (!mode) return undefined;
  const featureType = definition.kind === "transport-line" ? "line" : mode === "santander" ? "dock" : "stop";
  const clauses: unknown[] = ["all", ["==", ["get", "mode"], mode], ["==", ["get", "feature_type"], featureType]];
  if (mode === "bus" && atlasState.route && atlasState.route !== "all") {
    clauses.push(["==", ["to-string", ["get", definition.control?.routeProperty || "route_id"]], atlasState.route]);
  }
  return clauses as ExpressionSpecification;
}
...
function renderElectionControls(): void {
  const elections: Array<[AtlasState["election"], string]> = [
    ["general", "UK General · 4 July 2024"],
    ["local", "London boroughs · 5 May 2022"],
    ["mayor", "Mayor of London · 6 May 2021"],
    ["assembly", "London-wide Assembly · 6 May 2021"],
  ];
...
function renderEmptyFeaturePanel(): void {
  ...
  featurePanel.innerHTML =
    '<p class="feature-empty">Select an LSOA or map feature to see its details.</p>';
  updateDetailsToggleLabel();
}
...
function renderTransportControls(): void {
  const modes: Array<[string, string, string]> = [
    ["tube", "Underground", "London Underground"],
    ["dlr", "DLR", "Docklands Light Railway"],
    ["tram", "Trams", "London Trams"],
    ["overground", "Overground", "London Overground"],
    ["elizabeth-line", "Elizabeth line", "Elizabeth line"],
    ["bus", "Buses", "TfL bus routes and stops"],
    ["santander", "Santander Cycles", "Docking stations"],
  ];
  const routeOptions =
    manifest.layers.find((layer) => layer.control?.transportMode === "bus")?.control?.routes ?? [];
  ...
  controls.innerHTML = `
    ...
    <label class="select-label route-select">
      Bus route
      <select id="route-select" class="atlas-select" ${atlasState.transport.includes("bus") ? "" : "disabled"}>
    ...
    <p class="feature-empty">Static TfL snapshot. Select one bus route to reduce clutter.</p>`;
...
async function renderFeaturePanelForState(): Promise<void> {
  ...
  if (atlasState.group === "elections") {
    await renderLondonElectionPanel();
    return;
  }
  renderEmptyFeaturePanel();
}
...
async function renderLondonElectionPanel(): Promise<void> {
  currentFeatureTitle = "London elections";
  updateDetailsToggleLabel();
  featurePanel.innerHTML = renderLondonElectionCard();
}
...
function bindSectionCardActions(): void {
  ...
  featurePanel.querySelector<HTMLButtonElement>(".back-to-election")?.addEventListener("click", () => {
...
function resetView(): void {
  ...
  map.easeTo({
    center: [LONDON_CAMERA.lng, LONDON_CAMERA.lat],
    zoom: LONDON_CAMERA.zoom,
    bearing: 0,
    pitch: 0,
...
async function shareCurrentView(): Promise<void> {
  ...
  const title = atlasState.selectedSection ? "London LSOA" : "London Interactive Atlas";
  try {
