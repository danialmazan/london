...
layers <- list(
  ...
  layer("foreign-born", "population", "Foreign-born residents", "Census 2021 residents born outside the United Kingdom as a share of all usual residents. No citizenship or change layer is included.", "foreign_born_pct", lsoa, "percent", "%", "LSOA21", "2021-03-21", "lsoa21", palette_purple, short_label = "Foreign-born", data_status = "derived", tooltip_fields = list(tooltip("foreign_born_pct", "Born outside UK", "percent", "foreign_born_pct_percentile"))),

  layer("economic-activity", "education-work", "Economic activity", "Economically active residents aged 16+, including full-time students who were active.", "activity_rate_pct", lsoa, "percent", "%", "LSOA21", "2021-03-21", "lsoa21", palette_teal, short_label = "Activity", data_status = "derived", tooltip_fields = list(tooltip("activity_rate_pct", "Activity rate", "percent", "activity_rate_pct_percentile"))),
  layer("employment", "education-work", "Employment", "Residents aged 16+ in employment as a share of all residents aged 16+.", "employment_rate_pct", lsoa, "percent", "%", "LSOA21", "2021-03-21", "lsoa21", palette_blue, short_label = "Employment", data_status = "derived", tooltip_fields = list(tooltip("employment_rate_pct", "Employment rate", "percent", "employment_rate_pct_percentile"))),
  layer("unemployment", "education-work", "Unemployment", "Unemployed residents as a share of economically active residents aged 16+.", "unemployment_rate_pct", lsoa, "percent", "%", "LSOA21", "2021-03-21", "lsoa21", palette_orange, short_label = "Unemployment", data_status = "derived", tooltip_fields = list(tooltip("unemployment_rate_pct", "Unemployment rate", "percent", "unemployment_rate_pct_percentile"))),
  layer("level-4-plus", "education-work", "Level 4+ qualifications", "Residents aged 16+ whose highest qualification is Level 4 or above.", "level4plus_pct", lsoa, "percent", "%", "LSOA21", "2021-03-21", "lsoa21", palette_purple, short_label = "Level 4+", data_status = "derived", tooltip_fields = list(tooltip("level4plus_pct", "Level 4+", "percent", "level4plus_pct_percentile"))),
  layer("low-no-qualifications", "education-work", "Low or no qualifications", "Residents aged 16+ with no qualifications or Level 1/entry-level qualifications.", "low_no_qualifications_pct", lsoa, "percent", "%", "LSOA21", "2021-03-21", "lsoa21", palette_orange, short_label = "Low / none", data_status = "derived", tooltip_fields = list(tooltip("low_no_qualifications_pct", "Low or no qualifications", "percent", "low_no_qualifications_pct_percentile"))),
...
result_fields <- lapply(names(party_colours)[1:5], function(party) list(property = paste0("share_", party, "_PLACEHOLDER"), label = party_label(party), color = unname(party_colours[[party]])))

for (key in names(election_specs)) {
  spec <- election_specs[[key]]
  control <- list(election = key, party = "leading", results = lapply(result_fields, function(item) { item$property <- sub("PLACEHOLDER", key, item$property); item }))
  result_palette <- as.vector(rbind(names(party_colours), unname(party_colours)))
