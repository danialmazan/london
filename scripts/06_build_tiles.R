...
build_archive <- function(input, output, layer, minzoom = 8, maxzoom = 14, extra = character()) {
...
if (file.exists(file.path(processed_dir, "domestic-properties.geojsonseq"))) {
  build_archive("domestic-properties.geojsonseq", "domestic-properties.pmtiles", "domestic_properties", 11, 16, "--drop-densest-as-needed")
}
